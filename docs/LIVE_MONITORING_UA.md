...
flowchart LR
  ...
  B --> P["Anonymous avatar profile\nHttpOnly 256-bit token · TTL 30 days"]
...
|---|---|---|---|
| Browser file slots | Person, identity detail і фото речей не перезаписують один одного. Picker речей додає до попереднього selection. | `File` objects під час відкритої вкладки. | Preview і кількість файлів оновлюються після кожної дії. |
| Browser IndexedDB + upload preparation | Копіює оригінальні файли як Blob і text outfit після кожної зміни. Перед upload файли понад 18 MB або phone formats локально приводяться до JPEG ≤4096 px/≤18 MB. Bonus editorial still у core UI вимкнений. | Тільки на конкретному пристрої й origin `madeforthisjob.com`; consent не зберігається. | UI показує підготовку, потім фактичні uploaded MB/% через XHR. Кожен файл має окрему 10-minute аварійну межу. |
| Anonymous temp draft (`runtime/drafts`) | Після selection послідовно й один раз зберігає підготовлену server-side копію під випадковим browser ID з HttpOnly cookie. Submit чекає завершення цих upload, дозавантажує тільки відсутнє та створює run коротким JSON-запитом з наперед зафіксованим idempotency UUID. Якщо response загубився, reload знаходить той самий run, а не створює duplicate. | Person, identity, до 5 фото речей і text; без consent та original filenames. Sliding TTL 15 хвилин. | Cleanup запускається щохвилини й фізично видаляє directory після TTL. Explicit new run також одразу очищує temp draft. |
| Anonymous avatar profile (`runtime/profiles.sqlite`) | Зберігає browser-owned avatars, looks і run claims окремо від короткої чернетки. Новий look посилається на approved avatar source run і пропускає повторну avatar generation. | SQLite/WAL metadata; у cookie — випадковий 256-bit bearer token, у DB — лише SHA-256 verifier. Raw/output bytes лишаються в owner-bound immutable run directories. | Fixed expiry 30 днів, SameSite=Strict + Secure + HttpOnly, same-origin mutations, cross-profile 404. Delete/expiry ставить усі залежні runs у persistent deletion queue; running run дочищається після terminal state. |
...
| `RunService` / runner | Запускає checkpointed pipeline, публікує лише фактичні phase changes і після restart процесу знову піднімає persisted `QUEUED/RUNNING` jobs з їхніх checkpoints. | `run.json`, artifacts, receipts, QA evidence. | UI показує фактичний upload і сім server-milestones. Percent не «повзе» за таймером: він змінюється лише з новим persisted checkpoint. |
...
| Agent Supervisor | Щосекунди читає persisted run events, пояснює кожну server phase у monitor, знаходить stalls, створює дедуплікований incident fingerprint і за нового failure запускає окремий Codex bug-hunt. | `runtime/supervisor/state.json`, sanitized incident JSON і agent result; ніколи не передає runtime images, drafts або secrets. | Один agent одночасно, максимум 3 спроби на fingerprint, 8 хв для аналізу речей і 25 хв для generation stall. Agent може залишити patch у чистому source workspace, але не commit/push/deploy/restart. |
...
5. `http.response POST /api/draft/run 202` і `client.submit_response` — сервер читає вже збережені файли, ідемпотентно створює run з браузерним UUID і повертає той самий `run_id` на повторний request; повторного upload немає.
