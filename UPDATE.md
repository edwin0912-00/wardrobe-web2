...
| BETA-LOOK-ACTION-UI-002 | LOOK.06 → CHOICE.01 · Чіткий action dock після готового образу | codex-main | IN_PROGRESS | CODE + VISUAL QA | `web/public/index.html`; `web/public/app.js`; `web/public/scene-ui.js`; `web/public/result.css`; `test/web/profile-ui-flow.test.js`; `updates/codex-main.md` | Replace the equal-weight three-card row in the selected saved-look panel with one context-aware primary action and four distinct, truthful next-action modules: Improve (proposal), Background, Photoshoot, Fashion Video (blocked without transport), and Real-time Look. Preserve current working scene and Live paths; allow Background and Photoshoot to open their existing picker on their corresponding tab. No provider, camera, persistence, or generation-contract change. Prove keyboard/tap semantics and mobile/desktop fit before beta activation. |
