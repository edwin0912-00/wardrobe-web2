...
| BETA-LOOKPANEL-001 | PROFILE.03 · Панель збереженого образу не просвічує й не накриває свій вміст | claude-code-20260727-ui4f2a | READY_FOR_BETA_DEPLOY | CODE | `web/public/result.css`; `web/public/scene-cta.css`; `web/public/index.html`; `updates/claude-code-20260727-ui4f2a.md` | Відкрита панель образу непрозора (сітка карток більше не читається крізь неї), заголовок і всі три кнопки лишаються в межах панелі, а прокручуються лише списки сцен і фотосесій — перевірено замірами геометрії на 1280x800, 1366x768, 1440x900 і 1920x918, включно з девʼятьма сценами в списку. Точний протестований коміт `6e9cc68`; фокусна перевірка `node --test test/web/profile-ui-flow.test.js test/web/profile-navigation.test.js test/web/profile-app-integration.test.js` → 17/17 PASS. |
...
| BETA-PICKER-001 | UNIVERSE.02 · Превʼю Art Fashion + правдиві числа у вкладках | claude-code-20260727-ui4f2a | READY_FOR_BETA_DEPLOY | CODE | `src/web/scene-resolvers.js`; `web/public/index.html`; `web/public/scene-ui.js`; `test/web/editorial-preview-api.test.js`; `updates/claude-code-20260727-ui4f2a.md` | Превʼю `shoot.*` віддає доставлений кадр зйомки (мудкарту з `2a3e6dc`), а не контакт-шит; підписи вкладок беруться з даних, а не з константи — стандартних сцен 16, не 5. Доказ: `test/web/editorial-preview-api.test.js` і `test/contracts/scene-production-packs.test.js` (зарезервовані BETA-UNIVERSE-001) лишаються зеленими. РОЗВЕДЕННЯ КОЛІЗІЇ: `src/web/scene-resolvers.js` числиться за BETA-UNIVERSE-001 (antigravity-20260727-fb7a90), але той рядок `READY`, не `IN_PROGRESS` — fast rule 2 забороняє перекриття саме з `IN_PROGRESS`. Завдання дав Едвін напряму (fast rule 3). Зміна адитивна: додається лише перевага мудкарти в `editorialModePreview`, гілка шита і валідація контракту не змінюються, тому вона не конфліктує з метою BETA-UNIVERSE-001 (перевести два чоловічі юніти в продуктові стилі), а допомагає їй. Власник рядка може відкотити один коміт. Точний протестований коміт `dbc2442`; `node --test test/web/editorial-preview-api.test.js test/contracts/scene-production-packs.test.js` → 7/7 PASS. |
