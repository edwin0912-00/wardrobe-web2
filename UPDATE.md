...
| BETA-LOOK-NEXT-ACTIONS-001 | LOOK.06 → CHOICE.01–02 · П’ять живих карток після образу | unassigned | PROPOSED | PRODUCT + CODE | To be reserved after interaction review; no product path is reserved by this proposal. | Make **Покращити образ**, **Додати фон**, **Створити фотозйомку**, **Fashion Video**, and **Real-time Look** equal entry actions for one approved master-look. Each gets its own slow spotlight/glow language and a complexity intensity, with reduced-motion fallback and an honest unavailable state. Preserve existing truthful video block until real transport is ready. No UI, provider or beta change is authorized by this proposal. |
