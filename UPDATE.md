...
| BETA-LIVE-COMPLETE-001 | LIVE.01–04 · Камера, consent, session end і explicit capture | codex-live-20260727 | IN_PROGRESS | CODE + QA | `web/public/post-shoot-mvp.*`; `web/public/live-test-outfit.png`; `src/web/post-shoot-*.js`; `test/web/post-shoot-*.test.js`; `updates/codex-live-20260727.md` | Activate a one-page `/live` test with an automatically loaded outfit-only reference card from verified runtime garments, then keep browser camera local until explicit consent and preserve the bounded paid-session guard. No background recording and no consented provider call in QA. |
