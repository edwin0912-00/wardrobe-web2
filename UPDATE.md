...
| BETA-LOOK-NEXT-ACTIONS-001 | LOOK.06 → CHOICE.01–02 · П’ять живих карток після образу | codex-main | IN_PROGRESS | CODE + VISUAL QA | `web/public/choice-universe-preview.html`; `test/web/choice-universe-preview.test.js`; `updates/codex-main.md` | Build a standalone, non-functional beta preview screen for **Покращити образ**, **Додати фон**, **Створити фотозйомку**, **Fashion Video**, and **Real-time Look**. Each gets its own slow spotlight/glow and complexity intensity, with reduced-motion fallback and an honest non-live label. No provider call, camera permission, hidden navigation, or change to the saved-look flow. Test, activate the exact SHA and visually smoke the preview URL. |
