...
| BETA-VIDEO-SEEDANCE-001 | VIDEO.01–04 · Fashion video через Seedance 2.0 | claude-code-20260727-a3f1c8 | IN_PROGRESS | CODE | `config/motion-modes.json`; `schemas/motion-modes.schema.json`; `src/web/motion-modes.js`; `src/web/motion-service.js`; `src/web/motion-routes.js`; `src/providers/higgsfield-video-provider.js`; `test/video/**`; `updates/claude-code-20260727-a3f1c8.md` | The contract layer (motion-contract.js, already committed) and the service layer (this row) are named `motion-*`, not `video-*` -- the row's original declared paths were never built by the prior claimant, and the actual module follows the same naming as the file it wraps (motion-contract.js). src/providers/higgsfield-video-provider.js is claude-code-20260727-557761's own work (commit 6aa606d), kept in the row because the service is meant to call it through an injected executeMotion(context) adapter -- compatibility not yet verified. | Implement an idempotent Higgsfield CLI `seedance_2_0` route from one locked look/frame: explicit paid create, bounded status/retry, media QA, and a saved clip in the profile. Aspect and duration belong in the provider parameters, never in prompt prose. **Paths narrowed after a collision scan:** the original cell claimed `src/web/app.js` (held by `BETA-HEALTH-SEMANTICS-001`, opencloud), `web/public/post-shoot-*` (held twice by `codex-live-20260727`) and `test/web/**` (held by opencloud). None of those are touched here. Route registration and the client are delivered as a wire contract to their owners, the same way `BETA-LIVE-LOOKREF-001` was. Tests live in a new `test/video/`. |
...
| BETA-SCENE-JOURNEY-SMOKE-001 | BACKGROUND.01–02 + UNIVERSE.01–04 · Реальний smoke двох image-гілок | codex-main | BLOCKED_BY_SCENE_QA | QA | `ops/loops/create-universe-real-smoke-20260729/**`; `updates/codex-main.md` | Full approved look `8535a63d-7386-4a76-925b-f12143b137ae` is PASS. A real standard-background run `scene_c5d47bf2144e1b9ecfb236dd5d9378f27c106cf54d7f65d1` reached every Higgsfield image route: GPT Image 2 job `8387b8a5-372f-4090-a72f-02b635999323`, Nano Banana 2 job `9075d63c-0c4f-484b-9751-2a2ccd914ba6`, Nano Banana Pro job `737fda57-147a-4090-86fa-dba7be8e2582`. All three were honestly rejected for `FRAMING_AND_ANATOMY` (90.47–95.70% subject vs 74–78%, 1.41–1.88% headroom vs 8%) and `ITEM_FIDELITY` (exact shoe details; plus a malformed emblem in attempt 3). Transport is now proven; do not call std.* or shoot.* generation PASS until the scene-output contract has an evidence-preserving detail strategy. |
...
| BETA-FULL-LOOK-LOCK-001 | LOOK.04–06 + VIDEO.01 · Канонічні низ і взуття для повного approved look | codex-video-fidelity-20260728 | IN_PROGRESS | CODE + ASSETS + REAL PIPELINE QA | `tools/condition-dataset.mjs`; `inputs/zeely-test/dataset.manifest.json`; `inputs/zeely-test/outfits/locked-black-trousers.png`; `inputs/zeely-test/outfits/locked-black-trousers.source-magenta.png`; `artifacts/conditioning/garments/sneaker-black/**`; `artifacts/conditioning/garments/trousers-black/**`; `src/web/scene-service.js`; `src/web/editorial-shoot-bible.js`; `src/web/editorial-shoot-contract.js`; `src/web/run-service.js`; `src/web/first-appearance-lock.js`; `tools/build-product-release.mjs`; `tools/verify-product-release.mjs`; `test/conditioning/conditioning.test.mjs`; `test/web/approved-item-evidence.test.js`; `test/web/first-appearance-lock.test.js`; `test/web/editorial-activation-backend.test.js`; `test/release/product-release.test.js`; `updates/codex-video-fidelity-20260728.md` | **Direct operator re-open, 2026-07-29.** Implement and test the upstream first-appearance producer: from an approved full-body look, derive real lower-garment/footwear crops, obtain observed facts, create immutable hash-bound evidence and reuse it downstream. Existing registered items remain authoritative; a synthetic replacement is forbidden when item pixels are present. A release verifier exposed one direct prerequisite: cache-check actual ES module specifiers only; bundled package metadata is not a browser import. Producer output remains unchanged. Run a paid real-pipeline test only after deterministic and contract checks pass; record only public-safe hashes/outcomes. |
...
| BETA-QA-MANUAL-001 | QA · Ручний наскрізний QA-мануал для агентів | claude-code-20260727-ui4f2a | DONE | DOCS | `docs/qa/MANUAL-QA-MATRIX.md`; `updates/claude-code-20260727-ui4f2a.md` | v1.1.0: ДОДАНО §5 Fashion video (заблоковано — кнопка disabled, немає продуктового роуту; motion-contract.js готовий: 15 гейтів джоба/маршрутизації/receipt задокументовані, 0 генерацій для QA) і §6 Real-time Look (живий платний WebRTC-шлях decart/lucy-2-5/realtime: camera→confirm($0.20/5с)→POST /api/fal/realtime-token з гейтами MODEL_NOT_ALLOWED/PAID_SESSION_APPROVAL_REQUIRED/LUCY_PROVIDER_NOT_CONFIGURED, жорсткий автостоп рівно 5.0с). Мапа пайплайна (§2) і мінімальний прогін (§13) оновлені; решта розділів перенумерована 5→7 … 11→13, внутрішні §-посилання виправлені. Джерела: motion-contract.js, post-shoot-routes.js, post-shoot-mvp.js, index.html/app.js кнопки — всі file:line у тексті. |
