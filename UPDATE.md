...
| BETA-FULL-LOOK-LOCK-001 | LOOK.04–06 + VIDEO.01 · Канонічні низ і взуття для повного approved look | codex-video-fidelity-20260728 | HANDOFF · UPSTREAM_CAPTURE_REQUIRED | CODE + ASSETS | `tools/condition-dataset.mjs`; `inputs/zeely-test/dataset.manifest.json`; `inputs/zeely-test/outfits/locked-black-trousers.png`; `inputs/zeely-test/outfits/locked-black-trousers.source-magenta.png`; `artifacts/conditioning/garments/sneaker-black/**`; `artifacts/conditioning/garments/trousers-black/**`; `src/web/scene-service.js`; `src/web/editorial-shoot-bible.js`; `src/web/editorial-shoot-contract.js`; `test/conditioning/conditioning.test.mjs`; `test/web/approved-item-evidence.test.js`; `test/web/editorial-activation-backend.test.js`; `updates/codex-video-fidelity-20260728.md` | `58dd637` added disclosed locked trousers and real-source footwear dossiers; `982d170` removed its incorrect registry-only `FULL_LOOK_ITEMS_REQUIRED` pre-gate. Full suite evidence exists in `BETA-SUITE-GREEN-001` (427/427). **Handoff:** `RunService` / approved-look evidence must still implement the actual first-appearance crop+dossier+hash producer; do not reintroduce a block merely because a full-body look lacks a separate item row. |
