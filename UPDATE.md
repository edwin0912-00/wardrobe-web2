...
| BETA-MALE-UNITS-001 | UNIVERSE.01-02 · Ввести два чоловічі стилі (Охра, Жалюзі) в каталог | claude-code-20260727-ui4f2a | IN_PROGRESS | CODE | `docs/style-units/shoot.ochre_stage_tailoring/**` (КОЛІЗІЯ: BETA-UNIVERSE-001); `docs/style-units/shoot.shutter_amber_interior/**` (КОЛІЗІЯ: BETA-UNIVERSE-001); `src/web/scene-resolvers.js`; `src/web/editorial-shoot-bible.js`; `test/web/editorial-preview-api.test.js`; `updates/claude-code-20260727-ui4f2a.md` | РОЗВЕДЕННЯ: те саме пряме рішення оператора, що і для BETA-STYLE3-001 (не чекати antigravity, конфлікт — через ARBITRATION-STYLE3 до codex-main). Юніти мають виміряні палітри й 4 шити; добудовуються environment, colour_grade, person з закомічених source-кадрів, пишеться manifest.json з sha256, реєстрація +2 в обох списках. Мудкарти обох уже в репо. |
...
| BETA-FULL-LOOK-LOCK-001 | LOOK.04–06 + VIDEO.01 · Канонічні низ і взуття для повного approved look | codex-video-fidelity-20260728 | IN_PROGRESS | CODE + ASSETS | `tools/condition-dataset.mjs`; `inputs/zeely-test/dataset.manifest.json`; `inputs/zeely-test/outfits/locked-black-trousers.png`; `inputs/zeely-test/outfits/locked-black-trousers.source-magenta.png`; `artifacts/conditioning/garments/sneaker-black/**`; `artifacts/conditioning/garments/trousers-black/**`; `src/web/scene-service.js`; `src/web/editorial-shoot-bible.js`; `src/web/editorial-shoot-contract.js`; `test/conditioning/conditioning.test.mjs`; `test/web/approved-item-evidence.test.js`; `test/web/editorial-activation-backend.test.js`; `updates/codex-video-fidelity-20260728.md` | **Direct operator instruction.** Promote black sneakers from their real reference and create one disclosed synthetic black trouser item exactly once. Each must have a product view, observed/synthetic provenance, colour/construction/material/pattern dossier and SHA-256. Require a complete approved-item set before full-length video/editorial output; restore full-length coda only after the locks are bound. Never weaken ITEM_FIDELITY or substitute real footwear with generated pixels. |
