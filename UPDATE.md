...
| BETA-ATELIER-CHOICE-001 | LOOK.06 → CHOICE.01 · Інтерактивний atelier-прототип наступних дій | codex-main | IN_PROGRESS | CODE + VISUAL QA | `web/public/atelier-choice-prototype.html`; `test/web/atelier-choice-prototype.test.js`; `updates/codex-main.md` | Direct operator request: build a separate functional prototype where paper, swatches, pin, lamp, look slot and five action controls are DOM/CSS layers, not one raster image. Tapping a card must visibly select only that choice and explain it; no provider, camera, persistence or production-flow mutation. |
