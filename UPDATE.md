...
| BETA-LIVE-COMPLETE-001 | LIVE.01–04 · Камера, consent, session end і explicit capture | codex-live-20260727 | READY_FOR_BETA_DEPLOY | CODE + QA | `web/public/post-shoot-mvp.*`; `web/public/live-test-outfit.png`; `src/web/post-shoot-*.js`; `test/web/post-shoot-*.test.js`; `updates/codex-live-20260727.md` | One-page `/live` test now automatically loads a verified 1024×1024 garment-only reference card (green hoodie + black sneakers, no person). Focused tests 8/8 and browser smoke PASS: `Hoodie + sneakers · READY`, Camera available, paid Live disabled until camera/consent. No provider call was made. Deploy exact tested SHA, then repeat the non-billable browser smoke on the public URL. |
