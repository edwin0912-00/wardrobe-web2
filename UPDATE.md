...
| BETA-ROOT-UI-RECOVERY-001 | SHELL.01 · Beta не падає до старту pipeline | codex-main | READY | CODE + MANUAL QA | `web/public/index.html`; `web/public/app.js`; `web/public/*.js`; relevant focused tests; `updates/codex-main.md` | Repair the two observed root UI errors only: parse failure around `collisionContainer` and undefined inline `scrollToSection`. Deploy exact SHA, then independent manual QA must navigate 01–04 without fatal overlay before any product generation work resumes. |
