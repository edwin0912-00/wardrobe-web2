...
function safeData(value) {
  ...
  return Object.fromEntries(Object.entries(value).flatMap(([key, item]) => {
    ...
    if (typeof item === 'string') return [[key, item.slice(0, MAX_TEXT)]];
    if (Array.isArray(item)) return [[key, item.slice(0, 20).map((entry) => typeof entry === 'string' ? entry.slice(0, 200) : entry)]];
    return [];
  }));
}
