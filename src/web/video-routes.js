...
function publicVideoFailure(liveClip) {
  ...
  if (liveClip?.failureCode === 'VIDEO_PROVIDER_JOB_NOT_FOUND') {
    return 'Провайдер більше не бачить цю спробу. Нове відео не створювалося автоматично.';
  }
  if (liveClip?.failureCode === 'VIDEO_PROVIDER_JOB_FAILED') {
    if (['SUBMITTING', 'CREATED'].includes(liveClip?.automaticRetry?.state)) {
      return 'Провайдер завершив попередню спробу помилкою. Сервер уже запускає обмежену автоматичну спробу з тим самим затвердженим образом і стилем.';
    }
    return 'Провайдер завершив цю спробу помилкою. Відео не створилось; можна запустити нову спробу.';
  }
  if (liveClip?.failureCode === 'VIDEO_INPUT_MEDIA_IP_CHECK_PENDING') {
    return 'Вхідне медіа ще проходить перевірку. Сервер уже зробив одну безпечну автоматичну спробу; job не створився. Спробуйте ще раз через кілька секунд.';
  }
