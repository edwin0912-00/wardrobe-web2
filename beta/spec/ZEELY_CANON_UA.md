...
Історія редакцій цього файлу — у розділі 7. Читати її обов’язково перед тим,
як посилатися на правило: два правила вже змінювали зміст, і посилання на
застарілу редакцію виглядає як суперечність, якою воно не є.
...
- `CORE-AVATAR-002 MUST`: кадр фронтальний, нейтральний, повнозростовий — від маківки до підошов, зі взуттям повністю в кадрі. (Редакція 2026-07-27. До 2026-07-26 правило вимагало півростовий кадр до пояса або стегон; `QA-003` було переведено на повний зріст комітом `e8a5675`, а це правило-близнюк лишилося непереписаним і читалося як суперечність. Продукт увесь цей час робив повний зріст: `prompts/avatar.txt` і гейт `frontal_full_length_composition` у `src/qa/constants.mjs`.)
...
Статус розділу з 2026-07-27: `ART-003`–`ART-007` **ЗАМОРОЖЕНІ**. Вони описували
окремий showcase-ролик, не пов’язаний із затвердженою зйомкою. Того ж дня
з’явився чинний канон руху — `docs/VIDEO_LIVE_CANON_UA.md`, який вимагає
протилежного: будь-який рух стартує лише після `ART_SHOOT.05`, з конкретного
approved master-look і обраного style unit, і не має права міняти identity,
бренди чи стиль локації. Два несумісні відео-концепти не фінансуються
паралельно. Розморожувати лише після того, як `ART_SHOOT.05` реально віддасть
результат. `ART-001` і `ART-002` лишаються чинними.
...
- `1.0.0` — 2026-07-22, коміт `a4c1780`. Перша нормативна редакція з
  `ZEELY_TASK_SOURCE_UA.md`. Аватар описаний як півростовий і в
  `CORE-AVATAR-002`, і в `QA-003`.
- 2026-07-26, коміт `e8a5675`. `QA-003` переведено на повний зріст від маківки
  до підошов, бо континуїті сцен міряється саме так. `CORE-AVATAR-002` у тому
  коміті не змінили — з цього моменту файл ніс дві протилежні вимоги.
- `1.1.0` — 2026-07-27. `CORE-AVATAR-002` приведено у відповідність до `QA-003`
  і до коду. `ART-003`–`ART-007` заморожені на користь
  `docs/VIDEO_LIVE_CANON_UA.md`. Додано цей розділ, щоб редакція правила була
  видима без `git log`.
